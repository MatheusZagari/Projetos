...
public class desafioFilter {
    public static void main(String[] args) {
        ...
        Produto p2 = new Produto("Caderno", 30 , 0.45, 0);
        Produto p3 = new Produto("Notebook", 4899.89 , 0.32, 0);
        Produto p4 = new Produto("Monitor", 800 , 0.31, 0);
        Produto p5 = new Produto("iPad", 3100 , 0.29, 0);
        Produto p6 = new Produto("Relogio", 1900 , 0.12, 0);
        Produto p7 = new Produto("Impressora", 1200 , 0.40, 0);

        List<Produto> produtos = Arrays.asList(p1, p2, p3, p4, p5, p6, p7);

        //filter, filter, map

        Predicate<Produto> gradeDesconto = p -> p.desconto >= 0.3;
        ...
        produtos.stream().filter(gradeDesconto).filter(freteGratis).filter(produtoRelevante).map(chamadaPromocional).forEach(System.out::println);
    }
}
